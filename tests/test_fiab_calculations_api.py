...
class TestFiabCalculationsApi(unittest.TestCase):
    ...
    def test_single_unit_scenario(self):
        ...
        def create_calculation(test_context):
            ...
            fiab_account = FIABIdentifier(id="Client:/aapi/FIAB_TEST_HOLDINGS.ACCT")
            fiab_msl = "CLIENT:$$MSL_AAPI_TESTING.OFDB"
            fiab_dates = FIABDateParameters(startdate="20200618", enddate="20200618")
        ...
        def read_calculation_status(test_context):
            ...
            self.assertTrue(status_response[1] == 202 and (status_response[0].status in ("Pending", "InProgress", "Done")))
        ...
        def read_calculation_result(test_context):
            ...
            status_response = self.fiab_calculations_api.get_calculation_by_id(id=calculation_id,
                                                                               _return_http_data_only=False)
            self.assertEqual(status_response[1], 200, "Get calculation result should have succeeded")
            self.assertTrue(status_response[0].status == "Done", "Get calculation result should have Done status")
